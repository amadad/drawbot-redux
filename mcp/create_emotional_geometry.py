...
async def create_emotional_geometry():
    """Create emotional geometry PDF using MCP server and Dagger sandbox."""
    
    # Point to our MCP server
    client = Client("drawbot_mcp.py")
    
    async with client:
        print("=== Creating Emotional Geometry PDF ===\n")
        
        # Start a design session
        print("1. Starting design session...")
        result = await client.call_tool("start_design_session", {
            "session_id": "emotional_geometry",
            "width": 612,  # Letter width
            "height": 792,  # Letter height
            "description": "Scientific poster showing 6 emotional states as abstract geometric shapes"
        })
        print(f"Session started: {result}\n")
        
        # The full DrawBot code for the design
        emotional_geometry_code = '''import drawBot as db
import math
...
db.font("Helvetica", 10)
db.fill(0.3)
db.text("Emotional Intensity Scale:", (MARGIN, legend_y + 20))

# Draw gradient bar
gradient_width = 200
gradient_height = 10
gradient_x = MARGIN + 120

for i in range(gradient_width):
    intensity = i / gradient_width
    db.fill(intensity, intensity, intensity)
    db.rect(gradient_x + i, legend_y, 1, gradient_height)

# Scale labels
db.font("Helvetica", 8)
db.fill(0.4)
db.text("Low", (gradient_x - 5, legend_y - 5), align="right")
db.text("High", (gradient_x + gradient_width + 5, legend_y - 5), align="left")
...
'''
        ...
        # Execute the code in sandbox (using local for now)
        print("2. Executing DrawBot code in sandbox...")
        result = await client.call_tool("execute_drawbot_code", {
            "code": emotional_geometry_code,
            "session_id": "emotional_geometry",
            "output_name": "emotional_geometry.pdf",
            "sandbox": "dagger"  # Use Dagger for secure execution
        })
        ...
        if result_data.get("success"):
            ...
            print(f"  Sandbox used: {result_data.get('sandbox')}")
        else:
            print(f"✗ Error generating PDF: {result_data.get('error')}")
            if result_data.get('stderr'):
                print(f"  Error details: {result_data.get('stderr')}")
        
        return result


if __name__ == "__main__":
    asyncio.run(create_emotional_geometry())