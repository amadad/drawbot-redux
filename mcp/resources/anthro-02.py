...
class DesignSystem:
    ...
    def generate_drawbot_code(self, description, size):
        """Use Claude to generate DrawBot code tailored to the original canvas size"""
        width, height = size
...
Canvas size: {width} × {height} points.

Requirements:
- Use db.newPage({width}, {height})
...
- Keep the code clean and minimal."""
    ...
    def main(self):
        ...
        code = self.generate_drawbot_code(description, (width, height))
        success = self.execute_code(code, "output_v1a.png")

        if not success:
            return

        # Step 3: Critique and revision
        critique = self.get_critique(image_path, os.path.join(self.output_dir, "output_v1a.png"))
        ...
        self.execute_code(revised_code, "output_v2a.png")
