...
class DesignSystem:
    ...
    def execute_code(self, code, output_name):
        ...
        with open(temp_file, "w") as f:
            ...
            f.write(f'sys.path.insert(0, str(Path("{script_dir}")))\n')
            if drawbot_path.exists():
                f.write(f'sys.path.insert(0, str(Path("{drawbot_path}")))\n')
            f.write("import drawBot as db\n")
            f.write(f'os.chdir(str(Path("{script_dir}")))\n')
            f.write(code)
            if "saveImage" not in code:
                f.write(f'\ndb.saveImage(str(Path("{output_path}")))')

        try:
            # Use current Python interpreter with subprocess (safer than os.system)
            import subprocess
            python_path = sys.executable
            print(f"Using Python: {python_path}")
            print(f"Script directory: {script_dir}")
            print(f"DrawBot path: {drawbot_path}")
            result = subprocess.run([python_path, str(temp_file)], capture_output=True, text=True)
